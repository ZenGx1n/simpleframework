...
import org.simpleframework.aop.annitation.Aspect;
import org.simpleframework.aop.annitation.Order;
...
public class AspectWeaver {
    ...
    public void doAop() {
        ...
        for (Class<?> aspectClass : aspectSet) {
            if (verifyAspect(aspectClass)) {
                ...
                throw new RuntimeException("@Aspect and @Order have not been adder to the Aspect class," +
    ...
    private void categorizeAspect(Map<Class<? extends Annotation>, List<AspectInfo>> categorizedMap, Class<?> aspectClass) {
        ...
        if (!categorizedMap.containsKey(aspectTag.value())) {
            // 如果织入的joinpoint第一次出现，则以该joinpoint为key，已新创建的List<AspectInfo>为value
